...
const makeGroupsSocket = (config: LegacySocketConfig) => {
  ...
  return {
    ...
    groupCreate: async (title: string, participants: string[]) => {
      const response = (await groupQuery(
        "create",
        undefined,
        title,
        participants
      )) as WAGroupCreateResponse;
      ...
      try {
        ...
        logger.warn(
          `error in group creation: ${error}, switching gid & checking`
        );
    ...
    groupParticipantsUpdate: async (
      ...
      return Object.keys(result.participants || {}).map((jid) => ({
        jid,
        status: result.participants?.[jid],
      }));
    },
