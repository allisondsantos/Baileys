...
const makeChatsSocket = (config: LegacySocketConfig) => {
  ...
  const chatsDebounceTimeout = debouncedTimeout(10_000, () =>
    sendChatsQuery(1)
  );
    ...
    switch (updateType) {
      ...
      case "clear":
        if (node.content) {
          ...
          ev.emit("messages.delete", {
            keys: ids.map((id) => ({ id, remoteJid: jid })),
          });
        } else {
      ...
      case "pin":
        ev.emit("chats.update", [
          { id: jid, pin: attributes.pin ? +attributes.pin : null },
        ]);
        break;
      case "star":
      case "unstar":
  ...
  const applyingPresenceUpdate = (
    update: BinaryNode["attrs"]
  ): BaileysEventMap<any>["presence.update"] => {
  ...
  const chatRead = async (fromMessage: WAMessageKey, count: number) => {
    ...
    if (config.emitOwnEvents) {
      ev.emit("chats.update", [
        { id: fromMessage.remoteJid!, unreadCount: count < 0 ? -1 : 0 },
      ]);
    }
  };
  ...
  socketEvents.on(
    "CB:response,type:chat",
    async ({ content: data }: BinaryNode) => {
      ...
      if (Array.isArray(data)) {
        ...
        logger.info(
          `got ${chats.length} chats, extracted ${contacts.length} contacts with name`
        );
  ...
  return {
    ...
    getStatus: async (jid: string) => {
      const status: { status: string } = await query({
    ...
    async updateProfilePicture(jid: string, imgBuffer: Buffer) {
      ...
      const { eurl } = (await this.setQuery(
        [query],
        [WAMetric.picture, 136],
        tag
      )) as { eurl: string; status: number };
