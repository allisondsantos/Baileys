...
export const makeSocket = ({
  ...
  const generateMessageTag = (longTag: boolean = false) => {
    const tag = `${
      longTag ? referenceDateSeconds : referenceDateSeconds % 1000
    }.--${epoch}`;
  ...
  const sendNode = async ({
    ...
    if (binaryTag) {
      if (Array.isArray(json)) {
        throw new Boom("Expected BinaryNode with binary code", {
          statusCode: 400,
        });
      }

      if (!authInfo) {
        throw new Boom("No encryption/mac keys to encrypt node with", {
          statusCode: 400,
        });
      }
  ...
  const onMessageRecieved = (message: string | Buffer) => {
    if (message[0] === "!" || message[0] === "!".charCodeAt(0)) {
      ...
      Object.keys(l1).forEach((key) => {
        ...
        anyTriggered =
          ws.emit(`${DEF_CALLBACK_PREFIX}${l0},${key}:${l1[key]}`, json) ||
          anyTriggered;
        anyTriggered =
          ws.emit(`${DEF_CALLBACK_PREFIX}${l0},${key}`, json) || anyTriggered;
      });
      anyTriggered =
        ws.emit(`${DEF_CALLBACK_PREFIX}${l0},,${l2}`, json) || anyTriggered;
      anyTriggered =
        ws.emit(`${DEF_CALLBACK_PREFIX}${l0}`, json) || anyTriggered;

      if (!anyTriggered && logger.level === "debug") {
        logger.debug(
          { unhandled: true, tag: messageTag, fromMe: false, json },
          "communication recv"
        );
      }
    }
  ...
  const exitQueryIfResponseNotExpected = (
    tag: string,
    cancel: (error: Boom) => void
  ) => {
    ...
    const listener = ([, connected]) => {
      if (connected) {
        timeout = setTimeout(() => {
          logger.info(
            { tag },
            "cancelling wait for message as a response is no longer expected from the phone"
          );
  ...
  const waitForMessage = (
    ...
    if (ws.readyState !== ws.OPEN) {
      throw new Boom("Connection not open", {
        statusCode: DisconnectReason.connectionClosed,
      });
    }
      ...
      promise: (async () => {
        ...
        try {
          const result = await promiseTimeout(timeoutMs, (resolve, reject) => {
            ...
            cancelToken = () =>
              onErr(new Boom("Cancelled", { statusCode: 500 }));
  ...
  const query = async ({
    ...
    tag = tag || generateMessageTag(longTag);
    const { promise, cancelToken } = waitForMessage(
      tag,
      !!requiresPhoneConnection,
      timeoutMs
    );
    ...
    if (responseStatusCode === 599) {
      // the connection has gone bad
      end(
        new Boom("WA server overloaded", {
          statusCode: 599,
    ...
    if (expect200 && Math.floor(responseStatusCode / 100) !== 2) {
      ...
      throw new Boom(
        `Unexpected status in '${
          Array.isArray(json) ? json[0] : json?.tag || "query"
  ...
  const waitForSocketOpen = async () => {
    ...
    if (ws.readyState === ws.CLOSED || ws.readyState === ws.CLOSING) {
      throw new Boom("Connection Already Closed", {
        statusCode: DisconnectReason.connectionClosed,
      });
    }
  ...
  ws.on(PHONE_CONNECTION_CB, (json) => {
    if (!json[1]) {
        ...
        new Boom("Connection terminated by phone", {
          statusCode: DisconnectReason.connectionLost,
        })
      );
  ...
  ws.on("CB:Cmd,type:disconnect", (json) => {
    ...
    switch (kind) {
      ...
      new Boom(`Connection terminated by server: "${kind || "unknown"}"`, {
        statusCode: reason,
      })
    );
  });
  ...
  return {
    ...
    setQuery: async (
      nodes: BinaryNode[],
      binaryTag: WATag = [WAMetric.group, WAFlag.ignore],
      tag?: string
    ) => {
