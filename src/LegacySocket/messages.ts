...
const makeMessagesSocket = (config: LegacySocketConfig) => {
  ...
  const fetchMessagesFromWA = async (
    ...
    if (Array.isArray(content)) {
      return content.map((data) =>
        proto.WebMessageInfo.decode(data.content as Buffer)
      );
    }
    ...
    if (protocolMessage) {
      switch (protocolMessage.type) {
        ...
        case proto.Message.ProtocolMessage.Type.EPHEMERAL_SETTING:
          ...
          if (isJidGroup(jid)) {
            emitGroupUpdate({
              ephemeralDuration: protocolMessage.ephemeralExpiration || 0,
            });
          }
    ...
    if (message.messageStubType) {
      ...
      switch (message.messageStubType) {
        case WAMessageStubType.CHANGE_EPHEMERAL_SETTING:
          ...
          if (isJidGroup(jid)) {
            emitGroupUpdate({
              ephemeralDuration: +(message.messageStubParameters?.[0] || 0),
            });
          }
  ...
  const relayMessage = async (
    ...
    const isMsgToMe = areJidsSameUser(
      message.key.remoteJid!,
      state.legacy?.user?.id || ""
    );
    ...
    if (waitForAck) {
      ...
      promise
        ...
        .catch(() => {
          Sentry.captureException(WAMessageStatus.ERROR);
          emitUpdate(WAMessageStatus.ERROR);
        });
    }
  ...
  socketEvents.on("CB:action,add:unread", (json) =>
    messagesUpdate(json, false)
  );
  socketEvents.on("CB:action,add:before", (json) =>
    messagesUpdate(json, false)
  );
    ...
    if (Array.isArray(content)) {
      ...
      for (const { attrs: json } of content) {
        ...
        if (status) {
          ...
          logger.warn(
            { content, key },
            "got unknown status update for message"
          );
        }
      }
  ...
  return {
    ...
    loadMessageFromWA: async (jid: string, id: string) => {
      ...
      let messages = await fetchMessagesFromWA(jid, 1, {
        before: { id, fromMe: true },
      });
      if (!messages[0]) {
        messages = await fetchMessagesFromWA(jid, 1, {
          before: { id, fromMe: false },
        });
      }

      // the message after the loaded message is the message required
      const [actual] = await fetchMessagesFromWA(jid, 1, {
        after: messages[0] && messages[0].key,
      });
      return actual;
    },
        ...
        await setQuery([
          {
            ...
            content: [{ tag: "ephemeral", attrs: { value: value.toString() } }],
          },
        ]);
      } else {
