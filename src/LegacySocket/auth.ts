  ...
  const onOpen = async () => {
    ...
    if (canDoLogin) {
      ...
      const sendLoginReq = () => {
        if (state.connection === "open") {
          logger.warn(
            "Received login timeout req when state=open, ignoring..."
          );
          return;
        }
    ...
    let response = await Promise.race([
      ...
      ...(loginTag
        ? [socket.waitForMessage(loginTag, false, connectTimeoutMs).promise]
        : []),
    ]);
    initTimeout && clearTimeout(initTimeout);
    ...
    if (response.status && response.status !== 200) {
      throw new Boom("Unexpected error in login", {
        data: response,
    ...
    if (response[1].type === "upgrade_md_prod") {
      throw new Boom("Require multi-device edition", {
        statusCode: DisconnectReason.multideviceMismatch,
      });
    }

    // validate the new connection
    const { user, auth } = validateNewConnection(
      response[1],
      authInfo,
      curveKeys
