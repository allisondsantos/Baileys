...
export const makeChatsSocket = (config: SocketConfig) => {
  const { logger, markOnlineOnConnect, downloadHistory, fireInitQueries } =
    config;
    ...
    return {
      onMutation(mutation: ChatMutation) {
        processSyncAction(
          ...
          authState.creds.me!,
          recvChats
            ? { recvChats, accountSettings: authState.creds.accountSettings }
            : undefined,
          logger
        );
      },
  ...
  const resyncAppState = async (
    collections: readonly WAPatchName[],
    recvChats: InitialReceivedChatsState | undefined
  ) => {
          ...
          try {
            ...
            logger.info(
              ...
              `failed to sync state from version${
                isIrrecoverableError ? "" : ", removing and trying from scratch"
              }`
            );
            await authState.keys.set({
  ...
  const resyncMainAppState = async (ctx?: InitialReceivedChatsState) => {
    ...
    await processingMutex
      .mutex(() => resyncAppState(ALL_WA_PATCH_NAMES, ctx))
      .catch((err) => {
