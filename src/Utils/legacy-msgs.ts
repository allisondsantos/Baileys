  ...
  if (data.length) {
    ...
    if (typeof data === "string" || !possiblyEnc) {
      ...
      try {
        ...
        if (!macKey || !encKey) {
          throw new Boom(
            "recieved encrypted buffer when auth creds unavailable",
            { data: message, statusCode: DisconnectReason.badSession }
          );
        }
...
export const getAuthenticationCredsType = (
  creds: LegacyAuthenticationCreds | AuthenticationCreds
) => {
